...
object Day15 extends InputReader(15) with Day {
  ...
  def mapped(point: (Int, Int), value: Int, mult: (Int, Int)): ((Int, Int), Int) = {
    val p = (point._1 + (mult._1*smallGridX), point._2 + (mult._2*smallGridY))
    val n = value + mult._1 + mult._2
    p -> (if (n > 9) n - 9 else n)
  }

  val bigGrid = {
    val horizontal = (0 to 4).flatMap(i => smallGrid.map { case (p, v) => mapped(p, v, (i, 0)) })
    (0 to 4).flatMap(j => horizontal.map { case (p, v) => mapped(p, v, (0, j)) }).toMap
  }
    ...
    def loop(): Int = {
      ...
      if (next._2 == target._1 && next._3 == target._2) -1*next._1
        ...
        val adjacent = List(-1 -> 0, 1 -> 0, 0 -> -1, 0 -> 1).map {
          ...
          case p if !visited(p) => grid.get(p).map(v => (next._1 - v, p._1, p._2))
          case _ => None
        }
