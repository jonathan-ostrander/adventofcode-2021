...
object Day16 extends InputReader(16) with Day {
  ...
  object Packet {
    ...
    def parse(binary: String): (Packet, String) = {
      ...
      if (id == 4) {
        ...
        Packet(version, id, parseOperatorByLength(rest.drop(16).take(subpacketLength), Nil)) -> rest.drop(16 + subpacketLength)
      } else {
        val (operator, next) = parseOperatorByNumber(rest.drop(12), rest.drop(1).take(11).fromBinary.toInt)
        Packet(version, id, operator) -> next
      }
    }

    def parseLiteral(left: String, bitsSoFar: String): (Literal, String) = {
      val (head, bits, tail) = (left.head, bitsSoFar + left.tail.take(4), left.tail.drop(4))
      head match {
    ...
    def parseOperatorByLength(subpackets: String, packets: List[Packet]): Operator =
      subpackets match {
