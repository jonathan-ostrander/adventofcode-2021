...
object Day03 extends InputReader(3) with Day {
  ...
  implicit class BinaryString(val value: String) extends AnyVal { 
    def *(other: String): String = (Integer.parseInt(value, 2) * Integer.parseInt(other, 2)).toString
  }

  def partOne(): String = {
    val gamma = (0 until input.head.length).map(pos => bit(input, pos, true)).mkString
    val epsilon = (0 until input.head.length).map(pos => bit(input, pos, false)).mkString
    gamma * epsilon
  }
  def partTwo(): String = {
    def loop(left: List[String], pos: Int, mostCommon: Boolean): String = left match {
